...
void Graphic::Init(HWND hWnd) {
	...
	d3dpp.BackBufferHeight = r.bottom;// r.bottom + 1;
	d3dpp.BackBufferWidth = r.right;// r.right + 1;
	bufferHeight = r.bottom;
	bufferWidth = r.right;
	d3d->CreateDevice(
		D3DADAPTER_DEFAULT,
		D3DDEVTYPE_HAL,
...
void Graphic::Draw(float x, float y, LPDIRECT3DTEXTURE9 texture, RECT r, int alpha) {
	D3DXVECTOR3 position(x, y, 0);
	spriteHandler->Draw(texture, &r, NULL, &position, D3DCOLOR_ARGB(alpha, 255, 255, 255));
}
